...
// }
 if(count($error)==0){
    ...
    //  $tscore = $exam->test_input($_POST['tscore']);
     ...
     $private = $exam->test_input($exam->rsaPrivate($exampasscode));
     $public = $exam->test_input($exam->rsaPublic($private));
    // echo "Private = ". $private . " public $public";
    ...
    // exit;
     ...
     $status ="unavailable";
     $q = $exam->query("INSERT INTO examinations(display,rsa_private, rsa_public, current_term, status,subjectid, classid, description, examname, duration, startdate, enddate, examinationpin, visibility, examinationid)
     VALUES('$display','$private','$public', '$currentTerm','$status','$subject', '$classname', '$examinationdescription', '$examname', '$duration', '$startdate', '$enddate',
