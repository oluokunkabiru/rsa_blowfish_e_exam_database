<?php
    ...
    if(isset($_POST['addnewstaffbtn'])){
        ...
        if($_FILES['avatar']['size']==0){
            $error[] ="Please select staff passport for physical identification";
          }
          if($_FILES['avatar']['size']!=0){
    ...
    if(count($error)==0){
        ...
        if($user->checkExist($table, $column, $username) != ""){
            $username = $user->nextUsername($table, $column, $username);
        }
        ...
        $staff = new Users;
        $staffid = $staff->tableid("teachers");
        $q = $staff->query("INSERT INTO teachers (classid, userid, teacherid) VALUES('$class', '$id', '$staffid' ) ") ; 
        if($q){
            echo "<span class='text-success'>Staff added successfully</span>";
        }else{
    ...
    if(isset($_POST['addnewstafffilebtn'])){
        $errors = [];
            if($_FILES['stafffile']['size']==0){
                array_push($errors, "<span class='text-danger'>Please Select questions excel file</span>");
            }

            if($_FILES['stafffile']['size'] > 0){
                $target_file = basename($_FILES["stafffile"]["name"]);
            ...
            if(count($errors) ==0){
                ...
                // if(in_array($file_type, $ext)){
                    ...
                    $file = $_FILES['stafffile']['tmp_name'];
        ...
        //    exit;
            $handle = fopen($file, "r");
                        ...
                        <p><strong style="font-size:1.4em">Success</strong><br>Staffs uploaded successfully</p>.
                        </div>';
                    ...
                    if($surname !=""){
                        $files[$index][]=$filesop;
                        $role = "teacher";
                        $avatar = "staffs_avatar/default.png";
                        ...
                        VALUES('$avatar', '$sname', '$fname', '$lname', '$username','$role', '$password', '$id')");
                        $student = new Users;
                        $studentid = $student->tableid("teachers");
                        $q = $student->query("INSERT INTO teachers (classid, userid, teacherid) VALUES('$classid', '$id', '$studentid' ) ") ; 
                        if($q){
